...
namespace nlohmann {
	...
	inline void from_json(const json& j, OffsettParser::Netvars& x) {
		x.cs_gamerules_data = j.at("cs_gamerules_data").get<unsigned long>();
		x.m_armor_value = j.at("m_ArmorValue").get<unsigned long>();
		x.m_collision = j.at("m_Collision").get<unsigned long>();
		x.m_collision_group = j.at("m_CollisionGroup").get<unsigned long>();
		x.m_local = j.at("m_Local").get<unsigned long>();
		x.m_move_type = j.at("m_MoveType").get<unsigned long>();
		x.m_original_owner_xuid_high = j.at("m_OriginalOwnerXuidHigh").get<unsigned long>();
		x.m_original_owner_xuid_low = j.at("m_OriginalOwnerXuidLow").get<unsigned long>();
		x.m_survival_game_rule_decision_types = j.at("m_SurvivalGameRuleDecisionTypes").get<unsigned long>();
		x.m_survival_rules = j.at("m_SurvivalRules").get<unsigned long>();
		x.m_aim_punch_angle = j.at("m_aimPunchAngle").get<unsigned long>();
		x.m_aim_punch_angle_vel = j.at("m_aimPunchAngleVel").get<unsigned long>();
		x.m_ang_eye_angles_x = j.at("m_angEyeAnglesX").get<unsigned long>();
		x.m_ang_eye_angles_y = j.at("m_angEyeAnglesY").get<unsigned long>();
		x.m_b_bomb_planted = j.at("m_bBombPlanted").get<unsigned long>();
		x.m_b_freeze_period = j.at("m_bFreezePeriod").get<unsigned long>();
		x.m_b_gun_game_immunity = j.at("m_bGunGameImmunity").get<unsigned long>();
		x.m_b_has_defuser = j.at("m_bHasDefuser").get<unsigned long>();
		x.m_b_has_helmet = j.at("m_bHasHelmet").get<unsigned long>();
		x.m_b_in_reload = j.at("m_bInReload").get<unsigned long>();
		x.m_b_is_defusing = j.at("m_bIsDefusing").get<unsigned long>();
		x.m_b_is_queued_matchmaking = j.at("m_bIsQueuedMatchmaking").get<unsigned long>();
		x.m_b_is_scoped = j.at("m_bIsScoped").get<unsigned long>();
		x.m_b_is_valve_ds = j.at("m_bIsValveDS").get<unsigned long>();
		x.m_b_spotted = j.at("m_bSpotted").get<unsigned long>();
		x.m_b_spotted_by_mask = j.at("m_bSpottedByMask").get<unsigned long>();
		x.m_b_started_arming = j.at("m_bStartedArming").get<unsigned long>();
		x.m_clr_render = j.at("m_clrRender").get<unsigned long>();
		x.m_dw_bone_matrix = j.at("m_dwBoneMatrix").get<unsigned long>();
		x.m_f_accuracy_penalty = j.at("m_fAccuracyPenalty").get<unsigned long>();
		x.m_f_flags = j.at("m_fFlags").get<unsigned long>();
		x.m_fl_c4_blow = j.at("m_flC4Blow").get<unsigned long>();
		x.m_fl_defuse_count_down = j.at("m_flDefuseCountDown").get<unsigned long>();
		x.m_fl_defuse_length = j.at("m_flDefuseLength").get<unsigned long>();
		x.m_fl_fallback_wear = j.at("m_flFallbackWear").get<unsigned long>();
		x.m_fl_flash_duration = j.at("m_flFlashDuration").get<unsigned long>();
		x.m_fl_flash_max_alpha = j.at("m_flFlashMaxAlpha").get<unsigned long>();
		x.m_fl_last_bone_setup_time = j.at("m_flLastBoneSetupTime").get<unsigned long>();
		x.m_fl_lower_body_yaw_target = j.at("m_flLowerBodyYawTarget").get<unsigned long>();
		x.m_fl_next_attack = j.at("m_flNextAttack").get<unsigned long>();
		x.m_fl_next_primary_attack = j.at("m_flNextPrimaryAttack").get<unsigned long>();
		x.m_fl_simulation_time = j.at("m_flSimulationTime").get<unsigned long>();
		x.m_fl_timer_length = j.at("m_flTimerLength").get<unsigned long>();
		x.m_h_active_weapon = j.at("m_hActiveWeapon").get<unsigned long>();
		x.m_h_my_weapons = j.at("m_hMyWeapons").get<unsigned long>();
		x.m_h_observer_target = j.at("m_hObserverTarget").get<unsigned long>();
		x.m_h_owner = j.at("m_hOwner").get<unsigned long>();
		x.m_h_owner_entity = j.at("m_hOwnerEntity").get<unsigned long>();
		x.m_i_account_id = j.at("m_iAccountID").get<unsigned long>();
		x.m_i_clip1 = j.at("m_iClip1").get<unsigned long>();
		x.m_i_competitive_ranking = j.at("m_iCompetitiveRanking").get<unsigned long>();
		x.m_i_competitive_wins = j.at("m_iCompetitiveWins").get<unsigned long>();
		x.m_i_crosshair_id = j.at("m_iCrosshairId").get<unsigned long>();
		x.m_i_entity_quality = j.at("m_iEntityQuality").get<unsigned long>();
		x.m_i_fov = j.at("m_iFOV").get<unsigned long>();
		x.m_i_fov_start = j.at("m_iFOVStart").get<unsigned long>();
		x.m_i_glow_index = j.at("m_iGlowIndex").get<unsigned long>();
		x.m_i_health = j.at("m_iHealth").get<unsigned long>();
		x.m_i_item_definition_index = j.at("m_iItemDefinitionIndex").get<unsigned long>();
		x.m_i_item_id_high = j.at("m_iItemIDHigh").get<unsigned long>();
		x.m_i_most_recent_model_bone_counter = j.at("m_iMostRecentModelBoneCounter").get<unsigned long>();
		x.m_i_observer_mode = j.at("m_iObserverMode").get<unsigned long>();
		x.m_i_shots_fired = j.at("m_iShotsFired").get<unsigned long>();
		x.m_i_state = j.at("m_iState").get<unsigned long>();
		x.m_i_team_num = j.at("m_iTeamNum").get<unsigned long>();
		x.m_life_state = j.at("m_lifeState").get<unsigned long>();
		x.m_n_fallback_paint_kit = j.at("m_nFallbackPaintKit").get<unsigned long>();
		x.m_n_fallback_seed = j.at("m_nFallbackSeed").get<unsigned long>();
		x.m_n_fallback_stat_trak = j.at("m_nFallbackStatTrak").get<unsigned long>();
		x.m_n_force_bone = j.at("m_nForceBone").get<unsigned long>();
		x.m_n_tick_base = j.at("m_nTickBase").get<unsigned long>();
		x.m_rgfl_coordinate_frame = j.at("m_rgflCoordinateFrame").get<unsigned long>();
		x.m_sz_custom_name = j.at("m_szCustomName").get<unsigned long>();
		x.m_sz_last_place_name = j.at("m_szLastPlaceName").get<unsigned long>();
		x.m_third_person_view_angles = j.at("m_thirdPersonViewAngles").get<unsigned long>();
		x.m_vec_origin = j.at("m_vecOrigin").get<unsigned long>();
		x.m_vec_velocity = j.at("m_vecVelocity").get<unsigned long>();
		x.m_vec_view_offset = j.at("m_vecViewOffset").get<unsigned long>();
		x.m_view_punch_angle = j.at("m_viewPunchAngle").get<unsigned long>();
	}
	...
	inline void from_json(const json& j, OffsettParser::Offsets& x) {
		...
		x.signatures = j.at("signatures").get<OffsettParser::Signatures>();
		x.netvars = j.at("netvars").get<OffsettParser::Netvars>();
	}
